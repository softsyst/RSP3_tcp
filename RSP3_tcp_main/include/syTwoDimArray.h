...
class matrix : private vector<vector<T> >
{
   ...
   public:
	 ...
	 /////////////////////////
     size_type rows() const {return this->size(); }
	 ...
	 /////////////////////////
	 vector<T>& operator[] (const size_type& ix){return this->at(ix);}
	 /////////////////////////
	...
	void reorderRows(int row1, int row2, int order)
	{
		...
		auto it = begin(this);
		advance(it, first);
		colvect = this->at(first); //this colvect has to be inserted before "second"
...
{
    //typename typedef vector<T>::size_type size_type;
    for (typename vector<T>::size_type i = 0; i < m.rows(); i++)
    { s << endl << i <<" :  ";
      for (typename vector<T>::size_type j = 0; j < m.columns(); j++)
           s << m[i][j] <<" ";
    }
    s <<endl;
    return s;
